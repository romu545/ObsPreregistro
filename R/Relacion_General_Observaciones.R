...
Relacion_General_Observaciones <- function(Datos_Evaluar) {

  Consolidado_Observaciones <- purrr::reduce(
    .x = purrr::list_flatten(list(Datos_Evaluar = Datos_Evaluar, mget(ls(pattern = "Obs_", sorted = TRUE, envir = .GlobalEnv), envir = .GlobalEnv))),
